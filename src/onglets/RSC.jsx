  ...
  const createChartConfig = (label, title, color) => {
    return {
      ...
      useEffect(() => {
        let newSpeedChart;
        let newCadenceChart;
        let newStrideLengthChart;
        let newDistanceChart;

        if (speedChartContainer.current) {
            newSpeedChart = new Chart(speedChartContainer.current, speedChartConfig);
            setSpeedChart(newSpeedChart);
        }

        if (cadenceChartContainer.current) {
          newCadenceChart = new Chart(cadenceChartContainer.current, cadenceChartConfig);
          setCadenceChart(newCadenceChart);
        }
        if (strideLengthChartContainer.current) {
          newStrideLengthChart = new Chart(strideLengthChartContainer.current, strideLengthChartConfig);
          setStrideLengthChart(newStrideLengthChart);
        }
        if (distanceChartContainer.current) {
          newDistanceChart = new Chart(distanceChartContainer.current, distanceChartConfig);
        ...
        return () => {
          if (newSpeedChart) newSpeedChart.destroy();
          if (newCadenceChart) newCadenceChart.destroy();
          if (newStrideLengthChart) newStrideLengthChart.destroy();
          if (newDistanceChart) newDistanceChart.destroy();
          if (WriteIndicateCharacteristic && WriteIndicateCharacteristic.characteristic) {
